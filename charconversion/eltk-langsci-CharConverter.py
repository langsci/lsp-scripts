...
#
# Applied to work with the Language Science Press ecosystem by Felix Kopecky in January 2019.
# Application falls under the terms and conditions in LICENSE.TXT
...
"""
Inserted a simple application for TIPA->Unicode conversion.
"""
...
if __name__=='__main__':
    ...
    converter=CharConverter('tipa','uni')
    def replacewithconvert(match):
        match = match.group(1)
        return converter.convert(match)
    
    with open(sys.argv[1], 'r') as f:
	newf = re.sub(r'\\textipa{([^}]*)}',replacewithconvert,f.read())
        newfile = open('output.tex','w')
	newfile.write(newf)
	newfile.close()
	f.close()
